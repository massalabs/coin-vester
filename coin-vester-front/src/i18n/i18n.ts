...
class I18n {
  ...
  public t(key: string, interpolations?: Record<string, string>): string {
    ...
    if (!result) {
      console.error(`I18n: No translation found for key ${key}`);
    }
  ...
  private _getLang(): string {
    ...
    if (fromUrl) {
      localStorage.setItem('massa-coins-vester-lang', fromUrl);
    }

    return (
      fromUrl || localStorage.getItem('massa-coins-vester-lang') || INTL.EN_us
    );
  }
